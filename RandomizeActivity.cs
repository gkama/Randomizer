...
namespace Randomizer
{
	...
	public class RandomizeActivity : Activity, GestureDetector.IOnGestureListener
	{
		...
		protected override void OnCreate(Bundle bundle)
		{
			...
			SetContentView(Resource.Layout.RandomizeScreen);
			...
			aboutText.Text = "ABOUT: The application is designed to generate random letters based on the " +
				...
				"sound funny :). Each letter is generated based on the keypad. For example, if 0 is a number then it generates a random letter between A-Z. " +
							 "If the number 2 is used then the application will generate a random letter from A-C.\n" +
				"Below is the guide:\n0  -  A-Z\n1  -  A-Z\n2  -  A, B, C\n3  -  D, E, F\n" +
				"4  -  G, H, I\n5  -  J, K, L\n6  -  M, N, O\n7  -  P, Q, R, S\n8  -  T, U, V\n9  -  W, X, Y, Z\n" +
							 "Have fun!\n" +
			...
			translateButton.Click += (object sender, EventArgs e) => {
				...
				else{ translatedNumber = DigitTranslator.toRandomString (inputText.Text);
					
					if (translatedNumber.Length != categoryInt)
					{
						errorText.Text = "Incorrect Input! Try Again!";
					}
					else if (String.IsNullOrWhiteSpace(translatedNumber))
					{
						errorText.Text = "Incorrect Input! Try Again!";
					}
					else if (DigitTranslator.isDigit(translatedNumber) == false)
					{
						errorText.Text = "Incorrect Input! Try Again!";
					}
					else
